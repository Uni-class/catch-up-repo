...
const FileUploadPreview = forwardRef(
  (
    ...
    const [status, setStatus] = useState<
      'ready' | 'uploading' | 'error' | 'finished'
    >('ready');
    ...
    const uploadFile = () => {
      ...
      apiClient
        ...
        .then(() => {
          setProgress(100);
          setStatus('finished');
          if (uploadResultHandler) uploadResultHandler(true);
        })
        .catch((error) => {
          setStatus('error');
          if (uploadResultHandler) uploadResultHandler(false);
        });
    };
        ...
        {status === 'ready' ? (
          <Button
            ...
            onClick={(event) => {
              ...
              if (removeButtonClickHandler) removeButtonClickHandler();
            }}
          >
            <CloseIcon width={'2em'} />
          </Button>
